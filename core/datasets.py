...
class AutomotiveDataset(thd.IterableDataset):
    ...
    def __init__(self, config, *datasets):
        ...
        assert all([isinstance(d, thd.IterableDataset) for d in self.datasets]), "AutomotiveDataset only supports IterableDataset"
        self.config = config
        ...
        self.counts = 100*np.ones(len(self.datasets)) # Actual count of how many examples per dataset, sets to 100 so first examples don't massively change the percentages
